          ...
          {this.props.builds.filter((build) => {
            ...
            return (
              <div key={build._id} className='col-xs-12 col-sm-6 col-md-6 col-lg-4'>
                <BuildBox build={build} favorites={this.props.favorites} hideSettings />
              </div>
            );
          })}
...
export default connect(
  state => ({ builds: state.builds.builds, favorites: state.builds.favorites, selectedHero: state.talents.selectedHero }),
  { loadAllBuilds }
)(CommunityBuilds);
